...
3. **环境状态连续**：两种叶片等环境元素先于输入存在，玩家扰动已有状态，而不是每次操作重新生成一份特效；
...
- 动态底座攻击使用平滑 `ConstantForce`，采用距离倍率 `0.55`、时长倍率 `2.50` 和 Ease 系数 `1.0`；角色相对脚下板前移 `18.3cm`；
...
- 实机实例的完整 `58` 个 Override 字段已迁入 `DA_WorldInteractionConfig.Settings.RopeBridge`，Blueprint CDO / Actor Override 均关闭，resolved settings 与共享 Struct 完全一致；
- 最新专项连续执行两次 Attack01，最大峰值 `90.2cm/s / 206.0deg/s`；Landing 为 `309.3cm/s`，线速度比 `0.292`；
...
目标：让两种叶片成为持续存在的环境状态，并用同一批粒子反馈玩家移动、攻击、起跳、落地和爆炸。
...
- 原白色 Paper 外观已替换为 `T_Fol_Leafs_BC` 中的五瓣浅色叶片，内部 `Paper*` 命名作为兼容层保留；源商业图集不随公开仓库分发；
- `Caps Lock` 可在游戏内同步切换武器 Sweep 与轻量场可视化；两个 CVar 使用 `-1=DataAsset / 0=关 / 1=开`，关闭绘制不影响交互；
...
完整本地 `ExampleMap_Lumen` 已把展示箱替换为 `SM_WoodenBox1` 对应 Fracture 版本，并完成手动破裂确认；Runtime 使用可编辑 Soft Reference，资源缺失时回退自制 Demo 箱。商业 Mesh、派生 Geometry Collection、完整场景与 External Actor 不进入公开仓库。
...
| 静止、走跑、跳跃、攻击、爆炸经过杂物区 | 同一批两种叶片被扰动，落地后可再次唤醒，攻击沿刀路形成短尾流 | 规则完成，最终视觉与性能验收中 |
...
- Actor 与 Blueprint CDO 必须关闭 Override、绑定同一 `DA_WorldInteractionConfig`，resolved settings 必须与共享 RopeBridge Struct 完全一致；
...
- 攻击推进必须按配置保留并沿攻击方向可观测；当前动态底座基线为距离倍率 `0.55`、时长倍率 `2.50` 和 Ease 系数 `1.0`，相对脚下板前移 `18.3cm`，自动化最低门槛为 `15cm`；
