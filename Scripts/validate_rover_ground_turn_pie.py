...
def sample_run_turnback_motion(active):
    ...
    sample = {
        ...
        "reverse_speed": max(
            0.0,
            -(pawn.get_velocity().x * turnback["forward_x"]
              + pawn.get_velocity().y * turnback["forward_y"]),
        ),
    }
...
def validate_run_turnback_motion():
    ...
    if not require(
        ...
        return False
    if not require(
        final_sample["forward"] <= maximum_forward - 20.0,
        "RunTurnback did not accelerate away from its old travel direction "
        f"peak={maximum_forward:.1f}cm final={final_sample['forward']:.1f}cm",
    ):
        return False
    minimum_recovery_speed = max(350.0, turnback["entry_speed"] * 0.65)
    if not require(
        final_sample["reverse_speed"] >= minimum_recovery_speed,
        "RunTurnback did not build usable reverse speed before completion "
        f"speed={final_sample['reverse_speed']:.1f}",
    ):
        return False
    fast_recovery_speed = max(400.0, turnback["entry_speed"] * 0.75)
    fast_recovery_samples = [
        sample
        for sample in active_samples
        if sample["reverse_speed"] >= fast_recovery_speed
    ]
    if not require(
        bool(fast_recovery_samples),
        f"RunTurnback never accelerated to {fast_recovery_speed:.1f}cm/s while active",
    ):
        return False
    acceleration_time = fast_recovery_samples[0]["time"] - active_samples[0]["time"]
    if not require(
        acceleration_time <= RUN_TURNBACK_MAX_ACCELERATION_TIME_SECONDS,
        "RunTurnback accelerated too slowly "
