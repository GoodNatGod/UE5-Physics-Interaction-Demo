...
$scriptPath = Join-Path $PSScriptRoot "validate_physics_world_dual_lake_pie.py"
$editorCommand = Join-Path $EngineRoot "Engine\Binaries\Win64\UnrealEditor-Cmd.exe"
if (-not (Test-Path -LiteralPath $scriptPath -PathType Leaf)) {
    throw "Dual-Lake PIE script was not found: '$scriptPath'."
}
...
if (Get-Process -Name "UnrealEditor" -ErrorAction SilentlyContinue) {
    throw "Close Unreal Editor before running the dual-Lake PIE validation."
}

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$validationLog = Join-Path ([IO.Path]::GetTempPath()) "PhysicsWorld-DualLake-PIE-$timestamp-$PID.log"
...
if (-not $editorProcess.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "Dual-Lake PIE validation timed out after $TimeoutSeconds seconds; see '$validationLog'."
}
$editorProcess.WaitForExit()
$exitCode = $editorProcess.ExitCode

if (-not (Test-Path -LiteralPath $validationLog -PathType Leaf)) {
    throw "Dual-Lake PIE validation did not create its log: '$validationLog'."
}
$failure = Select-String -LiteralPath $validationLog -SimpleMatch "PHYSICS_WORLD_DUAL_LAKE_PIE_FAIL" | Select-Object -Last 1
if ($failure) {
    throw "Dual-Lake PIE validation failed: $($failure.Line.Trim()); see '$validationLog'."
}
if ($exitCode -ne 0) {
    throw "Unreal Editor exited with code $exitCode; see '$validationLog'."
}
$success = Select-String -LiteralPath $validationLog -SimpleMatch "PHYSICS_WORLD_DUAL_LAKE_PIE_OK" | Select-Object -Last 1
if (-not $success) {
    throw "Dual-Lake PIE validation produced no success marker; see '$validationLog'."
}
...
if ($runtimeErrors.Count -gt 0) {
    throw "Dual-Lake PIE validation found a runtime error: $($runtimeErrors[0].Line.Trim()); see '$validationLog'."
}

Write-Host $success.Line.Trim()
Write-Host "Dual-Lake PIE validation log: $validationLog"
