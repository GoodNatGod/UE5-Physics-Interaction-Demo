...
$secondaryTempRoot = [IO.Path]::GetFullPath("D:\tmp").TrimEnd('\') + '\'
$isSafeStageRoot = $stageRoot.StartsWith($tempRoot, [StringComparison]::OrdinalIgnoreCase) -or
    $stageRoot.StartsWith($localTempRoot, [StringComparison]::OrdinalIgnoreCase) -or
    $stageRoot.StartsWith($secondaryTempRoot, [StringComparison]::OrdinalIgnoreCase)
if (-not $isSafeStageRoot) {
    throw "StageRoot must be inside '$tempRoot', '$localTempRoot', or '$secondaryTempRoot'."
}
