...
$scriptPath = Join-Path $PSScriptRoot "validate_physics_world_melee_box.py"
if (-not (Test-Path -LiteralPath $scriptPath -PathType Leaf)) {
    throw "Physics World melee-box PIE script was not found: '$scriptPath'."
}
...
if (Get-Process -Name "UnrealEditor" -ErrorAction SilentlyContinue) {
    throw "Close Unreal Editor before running the Physics World melee-box PIE validation."
}

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$validationLog = Join-Path ([IO.Path]::GetTempPath()) "PhysicsWorld-MeleeBox-PIE-$timestamp-$PID.log"
...
if (-not $editorProcess.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "Physics World melee-box PIE validation timed out after $TimeoutSeconds seconds; see '$validationLog'."
}
$editorProcess.WaitForExit()
$exitCode = $editorProcess.ExitCode

if (-not (Test-Path -LiteralPath $validationLog -PathType Leaf)) {
    throw "Physics World melee-box PIE validation did not create its log: '$validationLog'."
}
$failure = Select-String -LiteralPath $validationLog -SimpleMatch "PHYSICS_WORLD_MELEE_BOX_PIE_FAIL" -ErrorAction SilentlyContinue | Select-Object -Last 1
if ($failure) {
    throw "Physics World melee-box PIE validation failed: $($failure.Line.Trim()); see '$validationLog'."
}
if ($exitCode -ne 0) {
    throw "Unreal Editor exited with code $exitCode; see '$validationLog'."
}

$success = Select-String -LiteralPath $validationLog -SimpleMatch "PHYSICS_WORLD_MELEE_BOX_PIE_OK" | Select-Object -Last 1
if (-not $success) {
    throw "Physics World melee-box PIE validation produced no success marker; see '$validationLog'."
}
...
if ($runtimeErrors.Count -gt 0) {
    throw "Physics World melee-box PIE validation found a runtime error: $($runtimeErrors[0].Line.Trim()); see '$validationLog'."
}

Write-Host $success.Line.Trim()
Write-Host "Physics World melee-box PIE validation log: $validationLog"
