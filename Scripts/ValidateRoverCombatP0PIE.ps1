...
$scriptPath = Join-Path $PSScriptRoot "validate_rover_combat_p0.py"
if (-not (Test-Path -LiteralPath $scriptPath -PathType Leaf)) {
    throw "Combat P0 PIE validation script was not found: '$scriptPath'."
}
...
if (Get-Process -Name "UnrealEditor" -ErrorAction SilentlyContinue) {
    throw "Close Unreal Editor before running the combat P0 PIE validation."
}

$timestamp = Get-Date -Format "yyyyMMdd-HHmmss"
$validationLog = Join-Path ([IO.Path]::GetTempPath()) "RoverReplica-CombatP0-PIE-$timestamp-$PID.log"
...
if (-not $editorProcess.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "Combat P0 PIE validation timed out after $TimeoutSeconds seconds; see '$validationLog'."
}
$editorProcess.WaitForExit()
$exitCode = $editorProcess.ExitCode

if (-not (Test-Path -LiteralPath $validationLog -PathType Leaf)) {
    throw "Combat P0 PIE validation did not create its log: '$validationLog'."
}
$failure = Select-String -LiteralPath $validationLog -SimpleMatch "ROVER_COMBAT_P0_PIE_FAIL" | Select-Object -Last 1
if ($failure) {
    throw "Combat P0 PIE validation failed: $($failure.Line.Trim()); see '$validationLog'."
}
if ($exitCode -ne 0) {
    throw "Unreal Editor exited with code $exitCode; see '$validationLog'."
}

$success = Select-String -LiteralPath $validationLog -SimpleMatch "ROVER_COMBAT_P0_PIE_OK" | Select-Object -Last 1
if (-not $success) {
    throw "Combat P0 PIE validation produced no success marker; see '$validationLog'."
}
...
if ($runtimeErrors.Count -gt 0) {
    throw "Combat P0 PIE validation found a runtime error: $($runtimeErrors[0].Line.Trim()); see '$validationLog'."
}

Write-Host $success.Line.Trim()
Write-Host "Combat P0 PIE validation log: $validationLog"
