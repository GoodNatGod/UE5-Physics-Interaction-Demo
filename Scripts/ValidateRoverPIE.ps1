...
$scriptPath = Join-Path $PSScriptRoot "validate_rover_pie.py"
if (-not (Test-Path -LiteralPath $scriptPath -PathType Leaf)) {
    throw "PIE validation script was not found: '$scriptPath'."
}
...
$validationLog = Join-Path ([IO.Path]::GetTempPath()) "RoverReplica-PIE-$timestamp-$PID.log"
$editorArguments = @(
...
if (-not $editorProcess.WaitForExit($TimeoutSeconds * 1000)) {
    ...
    throw "PIE validation timed out after $TimeoutSeconds seconds; see '$validationLog'."
}
$editorProcess.WaitForExit()
$exitCode = $editorProcess.ExitCode

if (-not (Test-Path -LiteralPath $validationLog -PathType Leaf)) {
    throw "PIE validation did not create its log: '$validationLog'."
}
if (Select-String -LiteralPath $validationLog -SimpleMatch "ROVER_PIE_VALIDATION_FAIL" -Quiet) {
    $failure = Select-String -LiteralPath $validationLog -SimpleMatch "ROVER_PIE_VALIDATION_FAIL" | Select-Object -Last 1
    throw "PIE validation failed: $($failure.Line.Trim()); see '$validationLog'."
}
if ($exitCode -ne 0) {
    throw "Unreal Editor exited with code $exitCode; see '$validationLog'."
}

$success = Select-String -LiteralPath $validationLog -SimpleMatch "ROVER_PIE_VALIDATION_OK" | Select-Object -Last 1
if (-not $success) {
    throw "PIE validation produced no success marker; see '$validationLog'."
}
...
if ($runtimeErrors.Count -gt 0) {
    throw "PIE validation found a project runtime error: $($runtimeErrors[0].Line.Trim()); see '$validationLog'."
}

Write-Host $success.Line.Trim()
Write-Host "PIE validation log: $validationLog"
