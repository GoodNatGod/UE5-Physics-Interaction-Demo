...
EXPECTED_GEOMETRY_COLLECTION = (
    ...
    "GC_PW_WoodenBox1_Fractured.GC_PW_WoodenBox1_Fractured"
)
...
def begin_validation(world):
    ...
    if anim_instance is None or not isinstance(anim_instance, unreal.RoverAnimInstance):
        ...
        state["combat_ready_since"] = 0.0
        return
    if state["combat_ready_since"] <= 0.0:
        state["combat_ready_since"] = time.monotonic()
        state["last"] = "waiting for combat animation warmup"
        return
    if time.monotonic() - state["combat_ready_since"] < 0.5:
        state["last"] = "waiting for combat animation warmup"
        return
...
def validate_attack_tick():
    ...
    if not state["saw_attack_started"]:
        state["last"] = f"waiting for Attack01 acknowledgement request={request_id}"
        return
    if combat.is_attacking():
        return
